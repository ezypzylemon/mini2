...
class VideoDisplay(QWidget):
    ...
    def keyPressEvent(self, event):
        global stop_flag, restart_flag, start_game_flag
        if event.key() == Qt.Key_Escape:  # ESC 키
            stop_flag = True
        elif event.key() == Qt.Key_R:  # R 키
            restart_flag = True
            stop_flag = True
        elif event.key() == Qt.Key_Space:  # Space 키
...
def camera_thread():
    ...
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # 해상도 높임
...
# 4) 메인 스레드
# --------------------
def main_loop(app, video_display):
    ...
    while not stop_flag:
        if not frame_queue.empty():
            ...
            # PyQt5를 통해 프레임 표시
            video_display.update_frame(frame)
